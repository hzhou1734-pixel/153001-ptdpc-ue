...
/**
 * @description 规范化图片地址
 * 后端返回的图片域名为 https(443)，该地址 SSL 握手失败导致图片无法加载。
 * 这里统一把后端上传目录（/uploads）转成站内绝对路径，开发环境由 vite 代理转发到后端 http(80)。
 * 非上传目录的第三方地址保持原样，避免影响 CDN 图片。
...
export const normalizeImageUrl = (url: string): string => {
    ...
    // 相对路径 uploads/xxx 补齐前导斜杠
    if (url.startsWith('uploads/')) return `/${url}`
    // 绝对地址 http(s)://domain/uploads/xxx -> /uploads/xxx
    if (/^https?:\/\//i.test(url)) {
        try {
