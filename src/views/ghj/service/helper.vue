    ...
    <div>
        ...
        >
            <div class="detail" v-if="detail.id">
                ...
                <div class="detail__item">
                    <span class="detail__label">销量：</span>
...
const handleStatus = async (row: any, status: string) => {
    await helperStatus({ id: row.id, status })
    ElMessage.success(status === '显示' ? '生活帮手已显示' : '生活帮手已隐藏')
}
